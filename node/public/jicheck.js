  ...
  let selectedDate = ""; // 선택된 날짜를 저장할 전역 변수
  ...
  flatpickr("#calendar", {
      ...
      onChange: async function(selectedDates, dateStr, instance) {
          ...
          selectedDate = dateStr; // 선택된 날짜를 전역 변수에 저장
  ...
  async function fetchSummary(selectedDate) {
      try {
          ...
          if (selectedDateOnly === currentDate) {
              ...
              const response = await fetch(`http://3.36.52.133:3000/getchatfroms3?file_name=chat_log_${selectedDate}.json`);
              ...
              if (data.error) {
                  ...
                  if (Array.isArray(parsedData.chat)) {
                      ...
                      const chatEntries = parsedData.chat.map(entry => `사용자: ${entry.message} \n 챗봇 : ${entry.response}`);
              ...
              const response = await fetch(`http://3.36.52.133:3000/getsummaryfroms3?file_name=chat_log_${selectedDate}.json`);
              const data = await response.json();
              console.log('getsummaryfroms3 response:', data);

              if (data.error) {
                  // S3에 파일이 없을 경우 /summarizechat 엔드포인트 호출
                  const summarizeResponse = await fetch(`http://3.36.52.133:3000/summarizechat?file_name=chat_log_${selectedDate}.json`);
...
async function fetchSummarySpeech(selectedDate){
  ...
  try {
    const response = await fetch("/texttospeech", {
      ...
      body: JSON.stringify({ summary: currentSummary, date }),
    });
